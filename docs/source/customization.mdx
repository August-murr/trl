...
You can now mix multiple datasets for training by providing a JSON config that details both the training and testing sets. The config should specify the path, name (or subset), split, column, and the proportion of each dataset you want to use.
